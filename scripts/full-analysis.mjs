...
// MERGE, never clobber. A subset run (`full-analysis.mjs ORAS`) used to
// overwrite the whole file with one name - so refreshing a single ticker
// silently destroyed the other nineteen, which is exactly the kind of quiet
// data loss this file exists to prevent. Named tickers now replace only their
// own rows; everything else is carried forward with its original timestamp.
if (argv.length && existsSync(`${base}.json`)) {
  const prior = JSON.parse(readFileSync(`${base}.json`, "utf8"));
  const fresh = new Set(payload.results.map((r) => r.ticker));
  const carried = (prior.results ?? []).filter((r) => !fresh.has(r.ticker));
  payload.results = [...payload.results, ...carried];
  payload._partialRun = `refreshed ${[...fresh].join(", ")}; ${carried.length} other names carried forward from the previous run`;
  const order = Object.values(GROUPS).flat();
  payload.results.sort((a, b) => order.indexOf(a.ticker) - order.indexOf(b.ticker));
}
...
for (const [g, list] of Object.entries(GROUPS)) {
  const rs = payload.results.filter((r) => r.group === g);
