...
| COMI, MASR, ORAS, CLHO | EODHD free tier | EODHD's free plan has no live intraday EGX data — confirmed live that `close` comes back `"NA"` for these — so it uses `previousClose` instead (flagged `is_estimate`), which is genuinely once-a-day data regardless of call frequency |
