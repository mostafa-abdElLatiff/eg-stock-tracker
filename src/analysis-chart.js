...
export function buildFundamentalsGlossary() {
  const terms = [
    ...
    ["Fibonacci extension", "Take a real swing low and swing high from actual price action (never invented), measure that leg's size, then project multiples of it (127.2%, 161.8%, 261.8% are the standard ratios) above the high as possible target levels. The idea: markets often keep moving in these same proportions after a breakout, though the reasoning is more \"enough traders watch these ratios that they become self-fulfilling\" than any deeper law. This project otherwise always prefers a real, already-tested resistance level over a derived one - Fib extensions only get used as a fallback when a stock breaks into genuinely new territory with no prior ceiling to reference (added Sept 2026 for ORAS, ABUK, EMFD specifically)."],
...
export function buildAnalysisCard(ticker, data, avgCost, avgCostIsLive = true) {
  ...
  <div class="analysis-card">
    <div class="analysis-head">
      ...
      ${
        data.verdictScore != null
          ? `<span class="pill">score ${data.verdictScore}/100</span>`
          : data.holdingScore != null
          ? `<span class="pill">holding ${data.holdingScore}/100</span>`
          : ""
      }
